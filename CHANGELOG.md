...
### Added
- **An asset can be renamed from the uploads panel, while it is still uploading** — the name was decided in the "Upload asset" dialog before a byte moved, which is the one moment the file is not on screen next to it, and a file dropped straight onto a folder skips that dialog entirely and is named after whatever the camera or the NLE called it. A row in the uploads panel now takes a click on the name, or on a pencil beside it, and edits in place. `asset_id` exists as soon as `/upload/initiate` answers, long before the transfer ends, and `/upload/complete` does not write the name again, so a rename at 40% survives the upload it is riding on. Two rows are deliberately not offered it: a queued row has no asset yet, so the edit would appear to work and write nothing; and a row rebuilt from upload history comes from `/me/assets`, which spans every project the user can see in whatever role they hold there, so the same edit could only answer 403. (#330 by @Lennart-Pingpong)
