...
- **An asset can be given a new version again after one was reclaimed or discarded** — version numbers were counted from the live versions only, while `uq_asset_versions_asset_version` spans `(asset_id, version_number)` whatever `deleted_at` says. So an asset that had had a version soft-deleted — by the stale-upload reaper, or by discarding an upload — handed the next upload a number the table was still holding, and every further attempt died on the constraint with a 500, permanently. Numbering now spans every version the asset has ever had, so the number after a discarded v2 is v3.
